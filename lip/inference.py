...
args.img_size = 96
video_path = Path() / 'base_videos'


def get_random_video():
    videos = os.listdir(args.face)
    if videos != None:
        index = np.random.randint(0, len(videos))
        video_dir = os.path.join(args.face, videos[index])
        return video_dir
    else:
        return -1
...
def face_detect(images):
    ...
    for rect, image in zip(predictions, images):
        if rect is None:
            cv2.imwrite('temp/faulty_frame.jpg',
...
def main():
    ...
    if not Path(args.audio).is_file():
        raise ValueError('--audio 指定音檔位置')
    if not Path(args.face).is_dir():
        raise ValueError('--face 指定基底影片存放資料夾位置')

    # elif args.face.split('.')[1] in ['jpg', 'png', 'jpeg']:
    #     full_frames = [cv2.imread(args.face)]
    #     fps = args.fps
    if get_random_video() == -1:
        raise ValueError('--face argument must be a valid path to video/image file')
    else:
        # video_stream = cv2.VideoCapture(args.face)
        video_stream = cv2.VideoCapture(get_random_video())
    ...
    for i, (img_batch, mel_batch, frames,
            coords) in enumerate(tqdm(gen,
    ...
    command = 'ffmpeg -y -i {} -i {} -strict -2 -q:v 1 {}'.format(args.audio, 'temp/result.avi',
                                                                  args.outfile)
