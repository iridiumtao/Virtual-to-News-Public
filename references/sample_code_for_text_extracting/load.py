    ...
    while stop != 0:
        ...
        for i in range(num_of_sen):
            ...
            for vertex in in_vertex:
                relation += ((edge_value_matrix[i][vertex] / sum(edge_value_matrix[vertex])) * vertex_weight_list[
                    vertex])  # vetexn has relationship with other vertex
...
def Cut_content(sentence_list):  # Create_sentence_list(with or without stopwords)
    ...
    for sen in sentence_list:
        ...
        with open(sw_path, 'r',
                  encoding='UTF-8') as file:  # https://github.com/goto456/stopwords/blob/master/cn_stopwords.txt
...
def Generate_content(Generate_quantity, Input_path, Output_path, delete_index_weight, rela_index_weight, Article):
    ...
    s = Summarization(text=content, sen_len=int(Generate_quantity), sen_rela_method=0, key_sen='',
                      delete_index_weight=delete_index_weight, rela_index_weight=rela_index_weight)
...
Generate_content(Generate_quantity=10, Input_path='Input/腸道', Output_path='./Output/', delete_index_weight=0.85,
                 rela_index_weight=0.85, Article='为何肠道健康如此重要.txt')
