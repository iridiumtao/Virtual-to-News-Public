...
for k in range(count):
    ...
    try:
        s = generate_articles(Generate_quantity=args.samples+1, Input_path=Input_path, Output_path=Output_path, Article=files[k],path_basetopic=topic_path)
    except:
        print("Error!")
        continue
...
# -----------------
    with tf.Session(config=tf_config, graph=tf.Graph()) as sess:
        ...
        for j in range(args.samples):
            ...
            pic(Output_path_cht=Output_path_cht + "image/", output_path_zh=Output_path_zh+"image/", uuid_sample=uuid_sample, keyword=topic + " + "+ keyword,paragraph=paragraph)
            #HTML
